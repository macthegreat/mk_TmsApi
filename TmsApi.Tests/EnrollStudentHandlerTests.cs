...
public class EnrollStudentHandlerTests
{
    ...
    .ExistsAsync(99, "CS-401", Arg.Any<CancellationToken>()).Returns(Task.FromResult(true));
        ...
        Assert.False(result.IsSuccess); Assert.Equal("already_enrolled", result.Error.Code);
    ...
    public async Task Handle_WhenCourseFull_ReturnsCapacityError()
    {
        ...
        .Select(i => new Enrollment { Id = i, CourseId = 1, Status = "pending" }).ToList()
        };
       ...
       .GetByCodeAsync("CS-401", Arg.Any<CancellationToken>()).Returns(Task.FromResult<Course?>(course));
        ...
        Assert.Equal("course_full", result.Error.Code); Assert.Equal(EnrollmentError.CourseFull("Advanced Web Dev", 35),
        result.Error);

        await enrollmentService.DidNotReceive().AddAsync(Arg.Any<Enrollment>(),
    ...
    public async Task Handle_SuccessfulPath_AddsEnrollmentOnce()
    {
        ...
        var course = new Course
        {
            ...
            Title = "Advanced Web Dev",
            MaxCapacity = 35,
            Enrollments = Enumerable.Range(1, 20).Select(i => new Enrollment { Id = i, CourseId = 1, Status = "pending" }).ToList()
        };
        courseService
        .GetByCodeAsync("CS-401", Arg.Any<CancellationToken>()).Returns(Task.FromResult<Course?>(course));
        enrollmentService
        ...
        await enrollmentService.Received(1).AddAsync(Arg.Is<Enrollment>(e => e.StudentId == 100 && e.CourseId == 1),
        Arg.Any<CancellationToken>());
