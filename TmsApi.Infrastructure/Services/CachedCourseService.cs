    ...
    var dbHit = false;
        var dto = await cache.GetOrCreateAsync(
            ...
            async (state, token) =>
            {
                ...
                var course = await state.service.GetByCodeAsync(state.code, token)?? throw new NotFoundException($"Course {state.code} not found");
               return new CourseDto(course.Id, course.Title, course.Code, course.MaxCapacity, course.Enrollments.Count);
            },
        ...
        var list = await cache.GetOrCreateAsync(
           ...
           service,
            async (state, token) =>
            {
                ...
                return courses.Select(c => new CourseDto(c.Id, c.Title, c.Code,c.MaxCapacity, c.Enrollments.Count)).ToList();
